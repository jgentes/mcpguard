...
import { saveCachedSchema, getCachedSchema, getIsolationConfigForMCP, } from '../utils/mcp-registry.js';
...
export class WorkerManager {
    ...
    findProjectRoot() {
        ...
        logger.warn({ cwd: process.cwd(), sourceFile: currentFile, searchedFrom: dirname(currentFile) }, 'Could not find project root (wrangler.toml or package.json), using cwd as fallback');
        return process.cwd();
    }
    startRPCServer() {
        if (this.rpcServer) {
    ...
    async loadMCP(mcpName, config) {
        ...
        try {
            ...
            const hasCachedSchemaAfterLoad = cachedAfterLoad && cachedAfterLoad.configHash === configHash;
            ...
            if (hasCachedSchemaAfterLoad) {
                ...
                logger.debug({ mcpId, mcpName, cacheKey, toolCount: tools.length, promptCount: prompts.length }, 'Cached MCP schema');
                saveCachedSchema({
                    mcpName,
                    configHash: this.hashConfig(mcpName, config),
                    tools,
