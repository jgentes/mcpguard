...
import type { MCPServerInfo, MCPTokenMetrics, MCPAssessmentError, TokenMetricsCache, TokenSavingsSummary, MCPSecurityConfig } from './types';
 ...
 */
async function assessURLBasedMCP(server: MCPServerInfo): Promise<URLAssessmentResult> {
  if (!server.url) {
    console.log(`Token Assessor: ${server.name} has no URL, skipping`);
    return { error: { type: 'unknown', message: 'No URL configured', errorAt: new Date().toISOString() } };
  }

  console.log(`Token Assessor: Assessing URL-based MCP ${server.name} at ${server.url}...`);
  console.log(`Token Assessor: Headers configured: ${server.headers ? Object.keys(server.headers).join(', ') : 'none'}`);
  ...
  try {
    ...
    console.log(`Token Assessor: Sending initialize request to ${server.url}...`);
    ...
    if (sessionId) {
      console.log(`Token Assessor: ${server.name} returned session ID: ${sessionId.substring(0, 20)}...`);
    } else {
    ...
    if (!initResponse.ok) {
      ...
      console.log(`Token Assessor: ${server.name} init failed with status ${statusCode}`);
      console.log(`Token Assessor: Response body: ${truncateBody(responseBody)}`);
    ...
    try {
      ...
      console.log(`Token Assessor: ${server.name} init response: ${truncateBody(initResponseBody, 200)}`);
    } catch {
      console.log(`Token Assessor: ${server.name} could not read init response body`);
    }
    ...
    console.log(`Token Assessor: Sending tools/list request to ${server.url}${sessionId ? ' with session ID' : ''}...`);
    ...
    if (!toolsResponse.ok) {
      ...
      console.log(`Token Assessor: ${server.name} tools/list failed with status ${statusCode}`);
      console.log(`Token Assessor: tools/list response body: ${truncateBody(responseBody)}`);
      ...
      const isSessionError = statusCode === 400 && (
    ...
    console.log(`Token Assessor: ${server.name} tools/list response: ${truncateBody(toolsResponseText, 200)}`);
    
    // Parse the response - handle both JSON-RPC and SSE formats
    let tools: MCPToolSchema[] = [];
    try {
      // Try parsing as regular JSON first
      const toolsResult = JSON.parse(toolsResponseText) as { result?: { tools?: MCPToolSchema[] } };
      tools = toolsResult.result?.tools || [];
    } catch {
      // If JSON parsing fails, try to extract from SSE format
      console.log(`Token Assessor: ${server.name} response is not plain JSON, trying SSE format...`);
    ...
    console.log(`Token Assessor: ${server.name} - ${tools.length} tools, ~${estimatedTokens} tokens`);
    return { metrics };
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    const rawError = error instanceof Error ? error.stack || error.message : String(error);
    console.log(`Token Assessor: Error assessing URL-based ${server.name}: ${errorMessage}`);
 ...
 */
export async function assessMCPTokens(server: MCPServerInfo): Promise<MCPTokenMetrics | null> {
 ...
 */
export async function assessMCPTokensWithError(server: MCPServerInfo): Promise<AssessmentResult> {
  ...
  return { error: { type: 'unknown', message: 'Failed to assess command-based MCP', errorAt: new Date().toISOString() } };
}

/**
 * Assess command-based MCP
 */
async function assessCommandBasedMCP(server: MCPServerInfo): Promise<MCPTokenMetrics | null> {

  // Can only assess command-based MCPs
  if (!server.command) {
    console.log(`Token Assessor: Skipping ${server.name} - no command or URL specified`);
    return null;
  }
  ...
  return new Promise((resolve) => {
    ...
    try {
      ...
      const command = process.platform === 'win32' && server.command === 'npx' 
        ? 'npx.cmd' 
        : server.command;
      ...
      mcpProcess.stderr?.on('data', (data: Buffer) => {
        ...
        console.log(`Token Assessor: ${server.name} stderr: ${data.toString().trim()}`);
      });
      ...
      mcpProcess.on('error', (error) => {
        if (!resolved) {
          resolved = true;
          clearTimeout(timeout);
          console.log(`Token Assessor: Error spawning ${server.name}: ${error.message}`);
          resolve(null);
        }
      });
      ...
      mcpProcess.on('exit', () => {
        if (!resolved) {
          resolved = true;
          clearTimeout(timeout);
          console.log(`Token Assessor: ${server.name} exited before assessment complete`);
          resolve(null);
        }
      });
    } catch (error) {
      if (!resolved) {
        resolved = true;
        clearTimeout(timeout);
        console.log(`Token Assessor: Exception assessing ${server.name}: ${error}`);
        resolve(null);
      }
    }
  });
...
export function calculateTokenSavings(
  ...
  console.log(`Token Savings: Calculating for ${servers.length} servers, ${configs.length} configs`);
  console.log(`Token Savings: Server names: ${servers.map(s => s.name).join(', ')}`);
  console.log(`Token Savings: Config mcpNames: ${configs.map(c => `${c.mcpName}(guarded:${c.isGuarded})`).join(', ')}`);
  console.log(`Token Savings: Token cache keys: ${Object.keys(tokenCache).join(', ')}`);
  ...
  for (const server of servers) {
    ...
    console.log(`Token Savings: Server "${server.name}" - config found: ${!!config}, isGuarded: ${isGuarded}, hasMetrics: ${!!metrics}`);
    ...
    if (isGuarded) {
      ...
      if (metrics) {
        ...
        console.log(`Token Savings: Adding ${metrics.estimatedTokens} tokens from assessed "${server.name}"`);
      } else {
        // Use default estimate for unassessed MCPs
        unassessedGuardedMCPs++;
        totalTokensWithoutGuard += DEFAULT_UNASSESSED_TOKENS;
        console.log(`Token Savings: Adding ${DEFAULT_UNASSESSED_TOKENS} estimated tokens for unassessed "${server.name}"`);
      }
    } else if (metrics) {
...
export async function testMCPConnection(
  ...
  if (!server.url && !server.command) {
    ...
    return {
      ...
      mcpName: server.name,
      steps,
      error: { type: 'unknown', message: 'No URL or command configured', errorAt: new Date().toISOString() },
  ...
  try {
    ...
    try {
      ...
      if (errorMsg.includes('abort') || errorMsg.includes('timeout')) {
        ...
        return {
          ...
          mcpName: server.name,
          steps,
          error: { type: 'timeout', message: 'Network connectivity test timed out', errorAt: new Date().toISOString() },
          durationMs: Date.now() - startTime,
        };
      }
    ...
    return {
      ...
      mcpName: server.name,
      steps,
      error: { type: 'unknown', message: 'Invalid URL format', errorAt: new Date().toISOString() },
      durationMs: Date.now() - startTime,
    };
  }
  ...
  try {
    ...
    if (!toolsResponse.ok) {
      ...
      const isSessionError = toolsResponse.status === 400 && (
    ...
    let toolCount = 0;
    try {
      const toolsResult = JSON.parse(toolsResponseBody) as { result?: { tools?: MCPToolSchema[] } };
