...
| Tool | Description |
|------|-------------|
| `execute_code` | Execute TypeScript code against an MCP (auto-loads MCPs if needed) |
| `disable_mcps` | Disable MCP servers in your IDE configuration |
| `search_mcp_tools` | Discover which MCPs are configured in your IDE |
| `load_mcp_server` | Manually load an MCP server |
| `list_available_mcps` | List all currently loaded MCP servers |
| `get_mcp_by_name` | Find a loaded MCP server by name |
...
For complex operations, use the `execute_code` tool:
...
```typescript
// Single execute_code call: ~750 tokens
