...
// Worker configuration
{
  ...
  // Optional (per MCP): allow outbound and enforce host allowlist
...
If you enable Network Access for a specific guarded MCP, MCP Guard can allow outbound `fetch()` but will enforce a **host allowlist** (and optional localhost access) inside the isolate.
